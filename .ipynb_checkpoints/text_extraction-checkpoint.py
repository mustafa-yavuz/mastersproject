...
# Further cleaning
# lemmatization
abstracts = list(map(lambda x: ' '.join([token.lemma_ for token in x]), a_list))
titles = list(map(lambda x: ' '.join([token.lemma_ for token in x]), t_list))

# removing PRON after lemmatization
abstracts = list(map(lambda x: x.replace("-PRON-",""), abstracts))
...
abstracts = list(map(lambda x: cleanin2(x), abstracts))
titles = list(map(lambda x: cleanin2(x), titles))
#######################################################
